...
env_file="${IMAPSYNC_ENV_FILE:-$script_dir/.env.imapsync}"
...
for arg in "$@"; do
  case "$arg" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: imapsync-dgitman.sh [--delete-source] [imapsync options]

Passwords are read from IMAPSYNC_PASSWORD1 and IMAPSYNC_PASSWORD2. They can be
injected by 1Password or loaded from the 1Password-managed .env.imapsync mount.
...
args=(
  imapsync
  --user1 davidmarkgitman@gmail.com
